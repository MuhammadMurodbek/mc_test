                    ...
                    <Row className='gap-3'>
                        {
                            loginFields.map((item:string,index:number)=>(
                                <Col key={index} xl={12} className="mt-3">
                                    <Field name={item} placeholder={item} className="w-100 py-2 px-1 border rounded"/>
                                    <ErrorMessage name={item} component='div' render={msg => <div className='text-danger small'>{msg}</div>}  />
                                </Col>
                            ))
                        }
